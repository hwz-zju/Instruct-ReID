...
__all__ = ['re_ranking', 'compute_jaccard_distance']
...
def re_ranking(q_g_dist, q_q_dist, g_g_dist, k1=20, k2=6, lambda_value=0.3):
    ...
    # adapt fusion
    # weight = np.exp(-lambda_value*original_dist)
    # weight[weight>1]=1
    # final_dist = jaccard_dist*weight+original_dist*(1-weight)
    del original_dist
    del V
    del jaccard_dist
...
def compute_jaccard_distance(target_features, k1=20, k2=6, cam_features=None, print_flag=True, search_option=3):
    end = time.time()
    ...
    if (cam_features is not None):
        ...
        cam_dist = cam_dist.expand(N, N) - 2 * torch.mm(cam_features, cam_features.t())
