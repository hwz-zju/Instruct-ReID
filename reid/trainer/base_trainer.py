...
class BaseTrainer(object):
    ...
    def train(self, data_loader, optimizer, lr_scheduler, test_loader=None, query=None, gallery=None):
        ...
        for i, inputs in enumerate(data_loader):
            ...
            is_start = i % batch_accumulation == 0
            is_end = (i + 1) % batch_accumulation == 0
            
            if is_start:
                current_iter = i // batch_accumulation + 1
            ...
            if is_start:
                optimizer.zero_grad()
            if self.fp16:
                with amp.scale_loss((loss / batch_accumulation), optimizer) as scaled_loss:
            ...
            if is_end:
                if self.this_task_info:
                    # import pdb;pdb.set_trace()
                    multitask_reduce_gradients(self.model, sync=True, ignore_list=['classifier'], task_grp=self.this_task_info.task_handle)
                optimizer.step()
                ...
                #     import pdb;pdb.set_trace()
                if current_iter % 10 == 0 and local_rank == 0:
