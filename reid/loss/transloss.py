...
class TransLoss(nn.Module):
    ...
    def forward(self, emb, emb_pos, emb_neg):
        ...
        dist_pos = torch.diag(euclidean_dist(emb, emb_pos))
        dist_neg = torch.diag(euclidean_dist(emb, emb_neg))
        # mat_dist = cosine_dist(emb, emb)
        
        
        y = torch.ones_like(dist_pos)
        loss = self.margin_loss(dist_neg, dist_pos, y)
        prec = (dist_neg.data > dist_pos.data).sum() * 1. / y.size(0)
        return loss, prec
