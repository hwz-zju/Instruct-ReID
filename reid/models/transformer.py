...
class Transformer_local(nn.Module):
    def __init__(self, num_classes, net_config):
        ...
        self.fusion = nn.Sequential(copy.deepcopy(self.base.blocks[-2]), copy.deepcopy(self.base.blocks[-1]))
        self.weight_block = copy.deepcopy(self.base.blocks[-1])
...
class Transformer_DualAttn_multi(nn.Module):
    def __init__(self, num_classes, net_config, num_cams=None, this_task_info=None):
        self.rearrange = True
        self.net_config = net_config
        super(Transformer_DualAttn_multi, self).__init__()
...
class Transformer_DualAttn(nn.Module):
    def __init__(self, num_classes, net_config, num_cams=None, this_task_info=None):
        self.net_config = net_config
        super(Transformer_DualAttn, self).__init__()
        if net_config.vit_type=='base':
    ...
    def forward(self, x, instruction, this_task_info=None, label=None, cam_label=None, view_label=None, forzen=False):
        ...
        project_feats = self.project(clot_feats_s.float().unsqueeze(1)).repeat(1,129,1)
        bio_feats = self.base(x, project_feats=project_feats)
