...
def specific_task_split_(task_spec, world_size, rank, tasks):
    ## sanity check
    ...
    for i,gs in enumerate(Ltask_sizes):
        ...
        Ltasks.append(dist.new_group(ranks=ranks))    ## The handle for each task
        Lroots.append(last)
    ...
    alltask_info.root_handles = dist.new_group(ranks=Lroots)
...
def specific_task_split(task_spec, world_size, rank, tasks):
    ...
    assert type(task_spec) is list
    assert all(map(lambda x: type(x) is int, task_spec))
    num_groups = len(task_spec)
    splits = np.sum(task_spec)
    assert world_size % splits == 0
    unit = int(world_size / splits)
    ## split
    task_spec = [x*unit for x in task_spec]
    groups = []
    roots = []
    last = 0
    group_info = edict()
    alltask_info = edict()
    for i,gs in enumerate(task_spec):
        ranks = list(map(int, np.arange(last, last+gs)))
        groups.append(dist.new_group(ranks=ranks))
        roots.append(last)
        if rank in ranks:
            group_info.task_handle = groups[-1]
            group_info.task_size = gs
            group_info.task_id = i
            group_info.task_rank = rank - last
            group_info.task_root_rank = last
        last += gs
    group_info.task_roots = roots
    group_info.num_groups = num_groups
    
    alltask_info.root_handles = dist.new_group(ranks=roots)
    alltask_info.task_sizes = task_spec
    alltask_info.task_root_ranks = roots
    alltask_info.task_num = num_groups
    
    return group_info, alltask_info
...
def get_taskinfo(args, world_size, rank):
    ...
    thistask_info.attt_file = tasks[thistask_info.task_id].get('attt_file', '')
