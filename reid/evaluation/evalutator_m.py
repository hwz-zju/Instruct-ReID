...
class Evaluator(object):
    ...
    def evaluate(self, data_loader, query, gallery):
        ...
        features, _, features_bio, features_clot = self.extract_features(self.model, data_loader, self.validate_feat)
        # distmat = self.pairwise_distance_t2i(features, query, gallery)
        distmat, x_f, y_f = self.pairwise_distance_t2i_f(features, features_bio, features_clot, query, gallery)
        ...
        for i, sims in enumerate(distmat):
            ...
            img_feats_topk = y_f[topk_idx]
            text_feats = x_f[i].repeat(128, 1)
            
            # bio_fusion, clot_fusion = self.model.module.mix_attn(img_feats_topk.to('cuda').detach(), text_feats.to('cuda').detach())
            # vl_embeddings = torch.cat([bio_fusion[:, 0], clot_fusion[:, 0]], dim=-1)
            # vl_f = self.model.module.fusion_feat_bn_cat(vl_embeddings.detach())
            vl_embeddings = self.model.module.mix_attn(img_feats_topk.to('cuda').detach(), text_feats.unsqueeze(dim=1).to('cuda').detach())[:,0]
            
            vl_f = vl_embeddings
            score = self.model.module.classifier_f(vl_f)[:, 1]
    ...
    @staticmethod
    def pairwise_distance_t2i_f(features, features_bio, features_clot, query=None, gallery=None):
        if query is None and gallery is None:
            n = len(features)
            x = torch.cat(list(features.values()))
            x = x.view(n, -1)
    ...
    def extract_features(model, data_loader, validate_feat, print_freq=50):
        ...
        with torch.no_grad():
            for i, data in enumerate(data_loader):
                ...
                # global_feat, clot_feats_s, feat, clot_feats_s, clot_feats
                bio_feats, clot_feats, outputs_img, text_features, text_features_n = model(imgs, clothes_imgs, cam_label=cam_ids, view_label=view_ids)
                ...
                for fname, clothes_fname, output, text_output, output_bio, output_clot, pid in zip(fnames, clothes_fnames, outputs, outputs_text_features, outputs_bio_feats, outputs_clot_feats, pids):
                    # import pdb;pdb.set_trace()
        ...
        return features, labels, features_bio, features_clot
